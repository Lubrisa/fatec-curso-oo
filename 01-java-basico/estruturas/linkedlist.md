...
No `LinkedList`, cada elemento não é guardado diretamente em um array. Em vez
disso, cada dado é encapsulado dentro de um pequeno objeto chamado **Nó**
(_Node_).

Cada nó carrega 3 campos:

- `value`: o dado armazenado (o número, a String, o objeto).
- `next`: um ponteiro para o próximo nó da sequência.
- `prev`: um ponteiro para o nó anterior da sequência.

O objeto principal `meuLinkedList` precisa apenas guardar 3 coisas:

- A referência para o **primeiro nó** (`first` / cabeça).
- A referência para o **último nó** (`last` / cauda).
- A quantidade de elementos (`size`).
...
1. Cria-se o novo nó `novoNo`.
2. Aponta `novoNo.next` para o nó que era o primeiro.
3. Aponta `primeiroAntigo.prev` para `novoNo`.
4. Atualiza `first = novoNo`.
...
- Para buscar `get(i)`, o Java parte da cabeça (`first`) e avança nó por nó até
  o índice desejado (ou parte de `last` andando para trás, se o índice estiver
...
- Para uma lista de 100.000 elementos, o programa fará bilhões de saltos de
  ponteiros, travando a aplicação!
...
- Um `LinkedList` aloca um **objeto Node completo** no Heap para cada elemento,
  contendo a referência do dado e dois ponteiros adicionais (`prev` e `next`),
  além do cabeçalho de memória de todo objeto Java.
